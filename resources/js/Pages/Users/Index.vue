<template>
    ...
    <BreezeAuthenticatedLayout>
        <template #header>
            <h2 class="flex justify-between items-center font-semibold text-sm text-gray-800 leading-tight">
                <div class="text-xl">All Users  &nbsp; <span class="text-sm">Total Record: {{total_users}}</span> </div>
                    <span>
                    <!-- <Link :href="route('users.create')" as="button" class="bg-indigo-800 p-2 rounded-xl text-white">Create user</Link> -->
                    </span>
            </h2>
        </template>
	...
	<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
		<div class="p-4">
            ...
            <Pagination class="p-3 mb-5 space-x-3" :links="users.links" />
				...
				<tbody>
					<tr v-for="user in users.data" :key="user.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span v-if="user.projects.length>0">{{user.projects.length}} Project(s):<br></span>
                            <span v-else>No Project<br></span>
                            <li v-for="project in user.projects"  :key="project.id" v-html="'-'+project.title"></li>
